...
function brandedStdout(chunk, ...rest) {
  ...
  // Runtime anti-delete chatter ("re-forwarded recovered media...") is written
  // via process.stdout.write by the obfuscated protection lib, bypassing the
  // console.log status-row mapping. The DM to sudo is the real signal, so drop
  // these per-event lines here; the boot "Anti-Delete ✓ active" row still shows
  // through the console.log path.
  if (text.includes('[anti-delete]')) return true
...
process.__saffulConsoleLogInterceptor = function (...args) {
  ...
  // Same QR rule as brandedStdout — the QR watcher in index.js re-prints
  // via raw stdout, so suppress the styled copy here to avoid double print.
  if (isQrOutput(text)) return
...
console.error = (...args) => {
  ...
  // Silence Node-core deprecation spam (e.g. DEP0040 punycode) and the
  // "Use node --trace-deprecation" hint line that follows it. Real errors
  // are never filtered — only this known noise class.
  if (/DeprecationWarning|\[DEP\d+\]|trace-deprecation/i.test(text)) return
  writeLine(`${RED}  ▣ ERROR                : ${text}${RESET}`)
}

console.warn = (...args) => {
  const text = rebrand(args.map(String).join(' '))
  // Warnings must respect the same hide list as everything else — e.g. the
  // known "Sharp native binary is unavailable" notice already lives in
  // HIDE_PATTERNS but was never applied on this path.
  if (shouldHide(text)) return
